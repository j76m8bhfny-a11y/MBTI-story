...
export class PosterPainter {
  ...
  constructor(config: PosterConfig) {
    ...
    // 获取 Canvas 上下文
    tthis.ctx = wx.createCanvasContext(this.canvasId, this.context);
  }
  ...
  exportToImage(): Promise<string> {
    return new Promise((resolve, reject) => {
      wx.canvasToTempFilePath({
        ...
        // 🔥 修改 3: 必须指定 destWidth/destHeight，否则可能输出空白或模糊
        destWidth: this.width * 2,  // 导出 2 倍清晰度
      ...
      }, this.context); // 🔥 修改 4: 这里必须传入 this.context
    });
  }
}
