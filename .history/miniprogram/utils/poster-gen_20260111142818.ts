...
export const drawPoster = async (canvas: any, ctx: any, data: any, ui: any) => {
  ...
  let stickerX = cardX + rpx(60); 
  
  stickers.slice(0, 3).forEach((item: any) => {
    ctx.save();
    // 简单排版，不做复杂旋转以免遮挡
    const textW = ctx.measureText(item.text).width + rpx(40);
    
    ctx.fillStyle = item.type === 'core' ? '#333' : '#FFF';
    drawRoundRectPath(ctx, stickerX, stubCursorY, textW, rpx(50), rpx(8));
    ctx.fill();
    ctx.lineWidth = 1; ctx.strokeStyle = '#333'; ctx.stroke();

    ctx.fillStyle = item.type === 'core' ? '#FFF' : '#333';
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(item.text, stickerX + textW/2, stubCursorY + rpx(25));
    
    ctx.restore();
    stickerX += textW + rpx(20);
  });

  stubCursorY += rpx(100);

  // 2. 总结文案
  ctx.fillStyle = '#555'; ctx.font = `${rpx(26)}px sans-serif`;
  ctx.textAlign = 'left';
  const textH = drawWrappedText(ctx, ui.poster.summary || '', cardX + rpx(40), stubCursorY, cardW - rpx(80), rpx(40));
  
  stubCursorY = textH + rpx(60);
