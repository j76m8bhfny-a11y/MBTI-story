...
Page<TestPageData, any>({
  ...
  onNextTap() {
    if (this.data.isAnimating) return;
    if (this.data.currentIndex >= 59) {
      // 所有题目完成，计算 MBTI 结果并跳转到结果页
      const result: MBTIResult = calculateMBTI(this.data.answers, questions);
      
      // 将结果存储到全局数据或缓存
      wx.setStorageSync('mbtiResult', result);
      wx.setStorageSync('testAnswers', this.data.answers);
  ...
  updateQuestion(index: number) {
    ...
    this.setData({
      ...
      currentAnswerText: question.opts[defaultIndex] || '',
      hasMoved: false
    });
  },
