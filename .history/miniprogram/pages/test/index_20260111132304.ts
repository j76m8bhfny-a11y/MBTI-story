...
const STAGE_MAP: Record<string, { start: number, end: number, nextStage: string | null }> = {
  '1': { start: 0,  end: 1,  nextStage: null }, // 0-9 是 10 道题
  '2': { start: 10, end: 19, nextStage: '3' },
  '3': { start: 20, end: 29, nextStage: '4' },
  '4': { start: 30, end: 39, nextStage: '5' }, // 第 31-40 题
  '5': { start: 40, end: 49, nextStage: '6' }, // 第 41-50 题
  '6': { start: 50, end: 59, nextStage: null }, // 第 51-60 题 (结束)
};
...
Page<TestPageData, any>({
  ...
  onLoad(options: { stage?: string }) {
    console.log('🚀 Test Page Loaded. Data:', questions); // 调试日志
    ...
    // ⚠️ 核心修复：调用处理函数，把 rawText 拆解为 UI 需要的 story/tag
    const rawQuestion = questions[startQIndex] as any;
    const processedQuestion = this._processQuestion(rawQuestion); 

    const defaultIndex = 3; // 默认中间

    // 初始化数据
    this.setData({
      currentQIndex: startQIndex,
      currentQuestion: processedQuestion, // 使用处理过的数据
      progressPercent: Math.round((startQIndex / questions.length) * 100),
      currentAnswerText: rawQuestion.opts[defaultIndex] || '',
      currentRulerIndex: defaultIndex,
      rulerValue: defaultIndex,
      // 确保第一次进入时不显示上一题的残留状态
      hasMoved: false,
      previewText: '',
      hasInteracted: false
  ...
  // 2️⃣ [新增] 数据处理 helper (请放在 updateQuestion 附近)
  _processQuestion(rawQ: any) {
    if (!rawQ) return {};
    
    const rawText = rawQ.txt || '';
    
    // A. 提取标签 (匹配 【xxx】)
    const tagMatch = rawText.match(/【(.*?)】/);
    const tag = tagMatch ? tagMatch[1] : '场景'; // 默认 Tag

    // B. 移除标签，获取正文
    let content = rawText.replace(/【.*?】/, '');

    // C. 智能拆分 Story 和 Question
    // 逻辑：以最后一个中文问号、句号或感叹号为界
    // 如果没有标点，直接取最后一段
    const parts = content.split(/[，。？！]/).filter((s: string) => s.trim() !== '');
    
    let question = '';
    let story = '';

    if (parts.length > 1) {
      question = parts.pop() + '?'; // 补一个问号（视觉优化）
      story = parts.join('，'); // 其余拼回去
    } else {
      question = content; // 只有一句话
      story = ''; // 没有背景故事
    }
  ...
  onNextTap() {
    ...
    if (!this.data.hasInteracted) {
      // 1. 触发震动
      wx.vibrateShort({ type: 'heavy' }); // 改用 heavy 更像报错反馈

      // 2. ⚠️ 核心：触发 CSS 抖动动画
      this.setData({ isShaking: true });

      // 3. 500ms 动画结束后重置状态，以便下次还能触发
      setTimeout(() => {
        this.setData({ isShaking: false });
      }, 500);

      // 4. (可选) 可以去掉 Toast，因为动画反馈已经足够强了，或者保留
      wx.showToast({
        title: '请先选个答案鸭~', 
        icon: 'none',
        duration: 1500
  ...
  updateQuestion(qIndex: number) {
    ...
    // ⚠️ 调用同一个处理函数
    const displayQ = this._processQuestion(nextQ);
    const defaultRulerIndex = 3;

    this.setData({
      currentQIndex: qIndex,
      currentQuestion: displayQ, // 赋值处理后的对象
      currentRulerIndex: defaultRulerIndex,
      rulerValue: defaultRulerIndex,
      hasAnswered: false,
