...
Page<TestPageData, any>({
  ...
  onCardTouchMove(e: WechatMiniprogram.TouchEvent) {
    ...
    if (diff < 0) {
      // 向左滑 -> 预览下一题
      // ⚠️ 只有当前不是最后一题时，才有 preview
      if (this.data.currentIndex < maxIndex) {
        previewIndex = currentIndex + 1;
        const nextQ = questions[this.data.currentIndex + 1];
        if (nextQ && nextQ.opts) {
          const nextTxt = nextQ.opts[currentIndex];
        ...
        // 最后一题往左滑，没有预览内容
        this.setData({ previewText: '查看结果' });
      }
    } else if (diff > 0) {
      // 向右滑 -> 预览上一题
      if (currentIndex > 0) {
        previewIndex = currentIndex - 1;
        const prevQ = questions[this.data.currentIndex - 1];
        if (prevQ && prevQ.opts) {
          const prevTxt = prevQ.opts[currentIndex];
          if (this.data.previewText !== prevTxt) {
            this.setData({ previewText: prevTxt });
          }
        }
      } else {
