...
Page<TestPageData, any>({
  ...
  flyOutAndSwitch(isRight: boolean, nextIndex: number) {
    ...
    // ⚠️ 修正：使用足够远的距离
    const flyDist = isRight ? FLY_DISTANCE : -FLY_DISTANCE;
    const flyRotate = isRight ? 25 : -25; // 稍微加大旋转角度，动作更夸张

    // A. 顶层卡片飞出
    this.setData({
      cardTransition: 'transition: transform 0.25s ease-in;',
  ...
  animateToNextQuestion(nextQIndex: number) {
    ...
    // ⚠️ 修正：固定向左飞，距离加倍
    const flyDist = -FLY_DISTANCE;

    // 1. 飞出动画 (固定向左飞，代表去未来)
    this.setData({
      cardTransition: 'transition: transform 0.25s ease-in;',
      cardTransform: `transform: translateX(${flyDist}px) rotate(-25deg); opacity: 0;`,
      // 视觉欺骗：底层卡片暂时不动或最大化
      backgroundTransform: 'transform: scale(1.0) translateY(0); transition: none;'
    });
